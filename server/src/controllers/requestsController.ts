...
/** Admin: requests to act on (everything except dismissed), newest first. */
export async function listAdminRequests(_req: Request, res: Response): Promise<void> {
  const requests = await ProjectRequest.find({ status: { $ne: 'dismissed' } })
    .sort({ createdAt: -1 })
    .limit(50);
  res.json({ requests: requests.map((r) => r.toJSON()) });
}
